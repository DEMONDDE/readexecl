...
public class ExeclRead {
     ...
     */
    public static List<String> readExcel(File execl) {
        ...
        try {
            // 获取Excel后缀名
            String fileType = execl.getName().substring(execl.getName().lastIndexOf(".") + 1, execl.getName().length());

            // 获取Excel工作簿
            inputStream = new FileInputStream(execl);
            ...
            logger.warning("解析Excel失败，文件名：" + execl.getName() + " 错误信息：" + e.getMessage());
            return null;
        } finally {
            try {
                if (null != workbook) {
...
//        resultData.setJob(job);
     ...
     * @param sql sql模板
     * @param variable 对应变量
     * @param value execl得到的数据
     * @return
     */
    public static List<String> changeto(String sql, String variable, List<String> value) {
        List<String> result = new ArrayList<>();
        String[] va = variable.split(",");
        for(String row:value) {
            String[] execlrow = row.split(",");
            String newsql = new String(sql);
            for (int j = 0; j < execlrow.length; j++) {
                newsql = newsql.replace(va[j], execlrow[j]);
            }
            result.add(newsql);
        }
        return result;
